...
export default function NotificationsPage() {
	...
	const linkFor = (n: Notification) => {
		...
		if (n.log_id) return "/concert/" + n.log_id.slice(0, 8) // fallback, l'id log non è un concert_id qui
		return "#"
	}
			...
			{notifs.length === 0 ? (
				...
				<div className="flex flex-col gap-1">
					{notifs.map((n) => {
						...
						const href = n.type === "follow" && actor?.username
							? "/u/" + actor.username
						...
						return (
							...
							>
								...
								{!n.read && (
									<div className="h-2 w-2 shrink-0 rounded-full bg-[#FF2D6B]" />
								)}
							</div>
						)
					})}
