...
function SignupInner() {
	...
	const handleSignup = async (e: React.FormEvent) => {
		...
		if (err) {
			if (err.code === "user_already_exists" || err.message?.includes("already registered")) {
				setError("Questa email è già registrata. Prova ad accedere."); return
			}
		...
		return (
			<main className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden p-6">
				...
				<div className="relative w-full max-w-sm">
					...
					<div className="flex flex-col items-center gap-3 border-l-2 border-[#FF2D6B]/30 bg-white/[0.02] py-6 pl-5 pr-5 text-center">
	...
	return (
		<main className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden p-6">
			...
			<div className="relative w-full max-w-sm">
				<div className="mb-8 text-center">
					<Link href="/" className="text-4xl font-bold [font-family:var(--font-display)]">Enc<span className="text-[#FF2D6B]">o</span>re</Link>
						...
						{cityOpen && citySuggestions.length > 0 && (
							<ul className="absolute z-20 mt-1 max-h-44 w-full overflow-y-auto border border-white/10 bg-[#1a1a22] shadow-xl">
								...
								{citySuggestions.map((c) => (
									<li key={c}><button type="button" onClick={() => { setCity(c); setCityOpen(false) }}
										className="w-full px-3 py-2 text-left text-sm text-white/70 hover:bg-white/10 hover:text-white transition">{c}</button></li>
								))}
							</ul>
						)}
					</div>
