...
import { Calendar, Navigation, MapPin, TrendingUp, SlidersHorizontal } from "lucide-react"
		...
		<main className="mx-auto max-w-6xl px-4 pt-8 pb-16 sm:px-6 sm:pt-12">
			...
			<div className="mb-8">
				{user ? (
					...
					<div>
						<p className="text-2xl font-bold [font-family:var(--font-display)]">
							{showAllItaly ? "Tutta Italia" : "Cosa succede vicino a te"}
						</p>
						...
						{/* ── SLOGAN PER NUOVI VISITATORI ── */}
						<p className="mt-3 text-xs text-white/30">
							Tieni traccia dei tuoi live, scopri chi c&apos;era con te.
						</p>
					</div>
				)}
			</div>
			...
			{/* ── PREVIEW DIARIO (solo non loggati) ── */}
			{!user && (
				<div className="mb-8 border-l-2 border-white/10 bg-white/[0.02] py-3 pl-5">
					<p className="text-xs font-semibold uppercase tracking-[0.2em] text-white/30 mb-1.5">Il tuo diario sarà così</p>
					<p className="text-sm text-white/40">
						12 concerti &nbsp;·&nbsp; media <span className="text-[#FFC24B]">4.1</span>/5 &nbsp;·&nbsp; top artista <span className="text-white/60">Radiohead</span> &nbsp;·&nbsp; <span className="text-white/60">Milano</span>
					</p>
				</div>
			)}

			{/* ── CONTROLLI RAGGIO (solo con GPS) ── */}
			{hasLocation && (
			...
			{/* ── IL TUO DIARIO (solo loggati con storico) ── */}
					...
					<div className="mb-5 flex items-center justify-between">
						<div className="flex items-center gap-3">
							...
							<span className="text-xs font-semibold uppercase tracking-[0.2em] text-[#FFC24B]">
								Prossimi {showAllItaly ? "in Italia" : userCity ? "vicino a " + userCity : ""}
							</span>
						</div>
						...
						{upcoming.slice(0, 12).map((c) => (
							<Link key={c.id} href={"/concert/" + c.id} className="group flex flex-col overflow-hidden border border-white/10 bg-white/[0.02] transition hover:border-white/25 hover:bg-white/[0.04]">
								<div className="relative aspect-[3/4] overflow-hidden bg-[#17171F]">
									...
									{c.priceMin != null && (
										<div className="absolute left-2 top-2 bg-black/60 backdrop-blur px-2 py-0.5 text-xs font-bold text-[#FFC24B]">da {c.priceMin}{c.priceCurrency === "EUR" ? "€" : ""}</div>
									)}
									{c.distanceKm != null && (
										<div className="absolute right-2 top-2 bg-black/60 backdrop-blur px-2 py-0.5 text-[10px] text-white/50">{Math.round(c.distanceKm)} km</div>
									)}
								</div>
								<div className="p-3">
			...
			{trending.length > 0 && (
				<section className="mb-12">
					<div className="mb-5 flex items-center justify-between">
						<div className="flex items-center gap-3">
							<div className="h-px w-6 bg-white/10" />
							<span className="text-xs font-semibold uppercase tracking-[0.2em] text-white/30">Top trend {new Date().getFullYear()}</span>
						</div>
