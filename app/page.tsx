...
import { Calendar, Navigation, MapPin, TrendingUp } from "lucide-react"
...
export default function Home() {
	...
	useEffect(() => {
		...
		const loadUpcoming = fetch("/api/ticketmaster-events?" + tmParams.toString()).then(r => r.ok ? r.json() : Promise.reject("Ticketmaster"))
		...
		Promise.allSettled([loadTrending, loadUpcoming, loadPersonal]).then((results) => {
			...
			// Upcoming
			const upcomingResult = results[1]
			if (upcomingResult.status === "fulfilled") {
				setUpcoming(((upcomingResult.value.events ?? []) as any[]).map((c: any) => ({
			...
			} else { newErrors.push("Concerti non disponibili") }

			// Personal
			const personalResult = results[2]
	...
	return (
		<main className="mx-auto max-w-6xl px-4 pt-8 pb-16 sm:px-6 sm:pt-12">
			<div className="mb-8">
				{user ? (
					...
					<div>
						...
						<p className="mt-1 text-sm text-white/40"><Link href="/login" className="text-[#FF2D6B] hover:underline">Accedi</Link> o concedi la geolocalizzazione.</p>
			...
			{upcoming.length > 0 && (
				<section className="mb-12">
					<div className="mb-5 flex items-center justify-between">
						<div className="flex items-center gap-3"><div className="h-px w-6 bg-[#FFC24B]/40" /><span className="text-xs font-semibold uppercase tracking-[0.2em] text-[#FFC24B]">Prossimi {userCity && !user ? "vicino a " + userCity : "in Italia"}</span></div>
					...
					<div className="grid grid-cols-3 gap-3 sm:grid-cols-4 md:grid-cols-6">
						{trending.map((a) => (
							<Link key={a.name} href={"/artist/" + (a.mbid || encodeURIComponent(a.name))} className="group flex flex-col gap-2 border border-white/10 bg-white/[0.02] p-3 transition hover:border-white/25 hover:bg-white/[0.04]">
								...
								<div className="text-center"><p className="truncate text-xs font-semibold text-white group-hover:text-[#FFC24B] transition-colors [font-family:var(--font-display)]">{a.name}</p><p className="mt-0.5 text-[10px] text-white/35"><TrendingUp className="inline h-3 w-3 mr-0.5" />{a.count} {a.count === 1 ? "live" : "live"}</p></div>
							</Link>
						))}
					</div>
				</section>
